...
public class OverlordCommands implements CommandExecutor {

	private GPP plugin;
	private Overlord overlord;
	public static Set<String> afk = new HashSet<String>();

	public OverlordCommands(GPP plugin, Overlord overlord) {
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		} else if (CommandUtil.cmdEquals(command, "next")) {
			Secretary secretary = Secretary.getSecretary(player);
			if (secretary!=null) {
				try {
					secretary.nextPage();
		...
		} else if (CommandUtil.cmdEquals(command, "prev")) {
			Secretary secretary = Secretary.getSecretary(player);
			if (secretary!=null) {
				try {
					secretary.previousPage();
				} catch (NoMorePagesException e) {
					player.sendMessage(e.getMessage());
				}
				return true;
		...
		} else if (CommandUtil.cmdEquals(command, "page")) {
			if (args.length==1) {
